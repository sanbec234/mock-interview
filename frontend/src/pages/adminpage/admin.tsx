...
const AdminPage: React.FC = () => {
  ...
  const handleDeleteClick = async () => {
    const isConfirmed = window.confirm(
      "Are you sure you want to delete the question bank? This action cannot be undone."
    );
  ...
  const handleDownloadClick = async () => {
    const isConfirmed = window.confirm(
      "Are you sure you want to Download all records? This action cannot be undone."
    );
    console.log(isConfirmed);
    if (!isConfirmed) {
      return;
